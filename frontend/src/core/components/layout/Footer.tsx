...
// Map each social icon name to its SVG path
...
export default function Footer() {
  return (
    <footer className="text-white bg-gray-900" role="contentinfo">
      <div className="py-12 container-page">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-4">
          ...
          {f.sections.map((sec: FooterSection) => (
            <div key={sec.title}>
              <h3 className="mb-4 text-lg font-semibold text-white">
                {sec.title}
              </h3>
              <ul className="space-y-2">
                {sec.links.map((ln: { name: string; href: string }) => (
                  <li key={ln.name}>
                    <Link
                      href={ln.href}
